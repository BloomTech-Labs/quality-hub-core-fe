...
export const handleSubmit = (e, formState, setDone, setOpen, createListing) => {
	e.preventDefault();
	createListing({ variables: formState })
		.then(res => {
			//Open 2nd modal
			setDone(true);
			//Close first modal
...
export const handleSave = (e, formState, closeWindow, createListing) => {
	e.preventDefault();
	let newFormState = { ...formState, isPublished: false };
	createListing({ variables: newFormState })
		.then(res => {
