...
export const handleSubmit = (e, formState, setDone, setOpen, updateListing) => {
	e.preventDefault();
	updateListing({ variables: formState })
		.then(res => {
			//Open 2nd modal
			setDone(true);
			//Close first modal
...
export const handleSave = (e, formState, closeWindow, updateListing) => {
	e.preventDefault();
	let newFormState = { ...formState, isPublished: false };
	updateListing({ variables: newFormState })
		.then(res => {
